...
from src.util.logging import EVT_CITATION_UNMATCHED, get_logger
from src.models.domain import Citation, RetrievedChunk
...
def ensure_citation_footer(text: str, chunks: list[RetrievedChunk]) -> str | None:
    """Guarantee that every grounded answer exposes the server-owned evidence labels.
    Returns the footer string to append, or None if citations are already inline."""
    if not chunks or re.search(r"\[\d+\]", text):
        return None
    references = "; ".join(
        f"[{index}] {c.guest}, *{c.episode_title}*" for index, c in enumerate(chunks, start=1)
    )
