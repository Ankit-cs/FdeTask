...
class ArtifactStreamFilter:
        ...
        while True:
            if not self._in_fence:
                ...
                if m and self._current is not None:
                    raw_content = self._buf[: m.start()].strip()
                    if self._current.kind == "html":
                        self._current.content = self._clean_html(raw_content)
                    else:
                        self._current.content = raw_content
                    self.artifacts.append(self._current)
                    self._buf = self._buf[m.end():]
                    self._in_fence = False
                    self._current = None
    ...
    def _clean_html(self, text: str) -> str:
        ...
        start = re.search(r"<!doctype\s+html|<html\b", candidate, flags=re.IGNORECASE)
        if start:
            candidate = candidate[start.start():]
        end = re.search(r"</html\s*>", candidate, flags=re.IGNORECASE)
        if end:
