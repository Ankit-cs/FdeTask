...
from src.routes.errors import NotFoundError, ProviderUnavailableError
...
async def summarize_session(session_id: UUID, request: Request) -> SummarizeResponse:
    import httpx
    state = request.app.state
    ...
    messages = await state.message_repo.list_for_session(session_id)
    if not messages:
        return SummarizeResponse(summary="Empty session")
        
    text = "Please summarize the following conversation into a short 4-5 word title:\n\n"
    for m in messages:
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                f"{state.settings.ollama_base_url}/api/generate",
                json={
    ...
    await state.session_repo.touch(session_id, title=summary)
