...
export default function ArtifactViewer({ artifactId, onClose }: Props) {
  ...
  useEffect(() => {
    if (artifact?.kind === "html" && rawSource === null) {
      getArtifact(artifactId, true)
        .then((a) => setRawSource(a.content))
        .catch((e) => setError(e.message));
    } else if (tab === "source" && rawSource === null) {
      getArtifact(artifactId, true)
        .then((a) => setRawSource(a.content))
        .catch((e) => setError(e.message));
    }
  }, [tab, rawSource, artifactId, artifact]);
      ...
      <header className="flex items-center gap-2 border-b border-slate-200 px-4 py-2.5">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {artifact?.kind === "html" && rawSource !== null && (
              <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium uppercase tracking-wider ${
                rawSource === artifact.content ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
              }`}>
                {rawSource === artifact.content ? "Verified Safe" : "Sanitized"}
              </span>
            )}
          </div>
