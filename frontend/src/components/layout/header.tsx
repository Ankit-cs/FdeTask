...
export function Header({ onOpenApiKey, onOpenMobileSidebar }: Props) {
  ...
  return (
    <header className="flex items-center gap-3 h-14 px-3 border-b border-white/[0.05] bg-[#0a0a0c] shrink-0">
      ...
      {/* Insert ProviderBadge for LLM status */}
      <ProviderBadge session={null} config={null} />

      <button
        onClick={onOpenApiKey}
        aria-label={apiKey ? 'API key connected, manage' : 'Connect API key'}
        className="flex items-center gap-2 text-[13px] px-2.5 py-1.5 rounded-md border border-white/[0.08] hover:border-white/[0.2] transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--color-accent)] focus-visible:outline-offset-2 ml-3"
      >
